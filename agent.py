...
def decide_action(obs, fault, urgency):
    ...
    if obs.distance_to_obstacle < 12:
        return "brake", 0.99, "critical obstacle distance"
    ...
    if fault == "engine_overheating" and urgency == "high":
        return "stop", 0.99, f"critical overheating ({obs.engine_temp})"
...
def decide_action_v2(obs, fault, urgency):
    ...
    if fault == "engine_overheating" and urgency == "high":
        if obs.distance_to_obstacle < 35 or obs.speed > 25:
            return "brake", 0.99, f"critical overheating with close obstacle ({obs.engine_temp})"
        return "request_service", 0.99, f"critical overheating ({obs.engine_temp})"
...
def choose_immediate_safe_action(obs):
    if (
        ...
        return "brake", 0.99, "highest-priority safety maneuver"
...
def agent_step(observation, task_name="autonomous_control"):
    ...
    if task_name == "fault_diagnosis":
        return Action(action_type="diagnose", value=0.99, reason=fault)
